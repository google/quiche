...
bool QuicFramer::ApplyHeaderProtection(EncryptionLevel level,
  ...
  if (!sample_reader.SkipBytes(sample_offset) ||
      !sample_reader.ReadStringPiece(&sample, kHPSampleLen)) {
    QUIC_DLOG(ERROR) << "Not enough bytes to sample: sample_offset "
                     << sample_offset << ", sample len: " << kHPSampleLen
  ...
  if (mask.empty()) {
    QUIC_DLOG(ERROR) << "Unable to generate header protection mask.";
    return false;
  }
  ...
  bool is_long_header = ((type_byte & FLAGS_LONG_HEADER) != 0);
  ...
  if (is_long_header && header_type == ZERO_RTT_PROTECTED &&
  ...
  if (!buffer_writer.Seek(pn_offset - 1) ||
      !buffer_reader.SkipBytes(pn_offset - 1)) {
    return false;
  }
...
bool QuicFramer::RemoveHeaderProtection(QuicDataReader* reader,
  ...
  QuicDataReader sample_reader(remaining_packet.data(),
                               remaining_packet.length());
  ...
  if (has_diversification_nonce) {
    ...
    if (!sample_reader.ReadStringPiece(&nonce, kDiversificationNonceSize)) {
  ...
  uint8_t bitmask = 0x1f;
  bool is_long_header = ((header->type_byte & 0x80) == 0x80);
  if (is_long_header) {
    bitmask = 0x0f;
  }
  uint8_t mask_byte;
  ...
  // Read the (protected) packet number from the reader.
  QuicStringPiece protected_pn;
  if (!reader->ReadStringPiece(&protected_pn, header->packet_number_length)) {
    QUIC_DVLOG(1) << "Failed to read PN from buffer";
    return false;
  }
  QuicDataReader protected_pn_reader(protected_pn.data(), protected_pn.size());
  char pn_buffer[PACKET_4BYTE_PACKET_NUMBER] = {};
  ...
  for (size_t i = 0; i < header->packet_number_length; ++i) {
    ...
    if (!mask_reader.ReadUInt8(&mask_byte) ||
        !protected_pn_reader.ReadUInt8(&protected_pn_byte) ||
