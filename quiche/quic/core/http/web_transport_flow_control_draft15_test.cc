...
using ::webtransport::StreamWriteOptions;
...
class FlowControlDraft15Test : public test::Draft15SessionTest {
 protected:
  ...
  webtransport::Stream* SetUpStreamWithDataLimit(QuicStreamId session_id,
                                                 uint64_t max_data) {
    WebTransportHttp3* wt = SetUpWebTransportDraft15ServerSession(
        session_id,
        /*initial_max_streams_uni=*/10,
        /*initial_max_streams_bidi=*/10,
        /*initial_max_data=*/max_data);
    if (wt == nullptr) return nullptr;

    session_->set_writev_consumes_all_data(true);
    EXPECT_CALL(*writer_, WritePacket(_, _, _, _, _, _))
        .WillRepeatedly(Return(WriteResult(WRITE_STATUS_OK, 0)));

    return wt->OpenOutgoingBidirectionalStream();
  }

  // Detects WT_DATA_BLOCKED capsule emission on the wire via before/after
  // deltas — same pattern as Section5_DataBlockedSentWhenLimitReached.
  uint64_t ConnectStreamBytesWritten(QuicStreamId session_id) {
    QuicSpdyStream* connect_stream = static_cast<QuicSpdyStream*>(
        session_->GetOrCreateStream(session_id));
    EXPECT_NE(connect_stream, nullptr);
    return connect_stream->stream_bytes_written();
  }

  // In the production path, raising WT_MAX_DATA marks associated streams
  // write-blocked; later, QuicStream::OnCanWrite fires and ultimately calls
  // adapter.OnCanWriteNewData. In this harness, TestStream::OnCanWrite is
  // a gmock stub that does not invoke the real base-class chain, so the
  // session pump cannot drive the adapter. This helper stands in for the
  // production chain's final step.
  void DeliverOnCanWriteTo(webtransport::Stream* stream) {
    QuicSpdyStream* quic_stream = static_cast<QuicSpdyStream*>(
        session_->GetOrCreateStream(stream->GetStreamId()));
    ASSERT_NE(quic_stream, nullptr);
    quic_stream->web_transport_stream_adapter()->OnCanWriteNewData();
  }
};

// Kept alive by the caller so post-call slice state can be inspected after
// a blocked Writev.
struct SliceHolder {
  QuicheMemSlice slice;
  absl::Span<QuicheMemSlice> AsSpan() { return absl::MakeSpan(&slice, 1); }
};
SliceHolder MakeSlice(absl::string_view data) {
  return SliceHolder{QuicheMemSlice::Copy(data)};
}
absl::Status WritevString(webtransport::Stream* stream,
                          absl::string_view data) {
  SliceHolder holder = MakeSlice(data);
  return stream->Writev(holder.AsSpan(),
                        webtransport::kDefaultStreamWriteOptions);
}
absl::Status WritevStringBufferUnconditionally(webtransport::Stream* stream,
                                               absl::string_view data) {
  SliceHolder holder = MakeSlice(data);
  StreamWriteOptions options;
  options.set_buffer_unconditionally(true);
  return stream->Writev(holder.AsSpan(), options);
}
...
TEST_P(FlowControlDraft15Test,
       WritevContract_ReturnsUnavailableWhenDataLimitExhausted) {
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;

  QuicStreamId session_id = GetNthClientInitiatedBidirectionalId(0);
  webtransport::Stream* stream = SetUpStreamWithDataLimit(session_id, 100);
  ASSERT_NE(stream, nullptr);

  QUICHE_EXPECT_OK(WritevString(stream, std::string(100, 'a')));

  EXPECT_THAT(WritevString(stream, "x"),
              StatusIs(absl::StatusCode::kUnavailable));
}

TEST_P(FlowControlDraft15Test,
       WritevContract_DoesNotReturnResourceExhausted) {
  // The webtransport::Stream contract permits only Ok or Unavailable for
  // the retryable blocked-write case — ResourceExhausted would take
  // callers that branch on status code off the retry path.
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  absl::Status status = WritevString(stream, "x");
  ASSERT_FALSE(status.ok());
  EXPECT_NE(status.code(), absl::StatusCode::kResourceExhausted)
      << "Writev() must never return ResourceExhausted — the "
         "webtransport::Stream contract only permits Ok or Unavailable "
         "for the retryable blocked-write case.";
  EXPECT_EQ(status.code(), absl::StatusCode::kUnavailable);
}

TEST_P(FlowControlDraft15Test,
       WritevContract_WriteLargerThanRemainingWindowReturnsUnavailable) {
  // CanWrite() is a boolean floor (≥1 byte of window), not a size
  // predicate. A write larger than the remaining window must still return
  // Unavailable rather than silently writing a partial amount.
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  QUICHE_EXPECT_OK(WritevString(stream, std::string(90, 'a')));
  EXPECT_TRUE(stream->CanWrite());

  EXPECT_THAT(WritevString(stream, std::string(20, 'b')),
              StatusIs(absl::StatusCode::kUnavailable));
}

TEST_P(FlowControlDraft15Test,
       WritevContract_BufferUnconditionallyDoesNotBypassDataLimit) {
  // `buffer_unconditionally=true` bypasses the QUIC stream-level buffer
  // threshold, but must not bypass the WT session data limit: doing so
  // would put bytes on the wire in excess of WT_MAX_DATA (§5.4).
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  EXPECT_THAT(WritevStringBufferUnconditionally(stream, "x"),
              StatusIs(absl::StatusCode::kUnavailable));
}

TEST_P(FlowControlDraft15Test,
       WritevContract_CanWriteFalseWhenDataLimitExhausted) {
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  EXPECT_FALSE(stream->CanWrite())
      << "CanWrite() must return false once the WT session data limit is "
         "exhausted — regardless of stream-level writability.";
}

TEST_P(FlowControlDraft15Test,
       WritevContract_CanWriteTrueAfterWtMaxDataIncrease) {
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  InjectCapsuleOnConnectStream(
      session_id, Capsule(WebTransportMaxDataCapsule{200}));

  EXPECT_TRUE(stream->CanWrite());
  QUICHE_EXPECT_OK(WritevString(stream, std::string(50, 'b')));
}

TEST_P(FlowControlDraft15Test,
       WritevContract_OnCanWriteFiresAfterWtMaxDataIncrease) {
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  auto visitor = std::make_unique<
      testing::NiceMock<webtransport::test::MockStreamVisitor>>();
  webtransport::test::MockStreamVisitor* visitor_raw = visitor.get();
  stream->SetVisitor(std::move(visitor));

  QUICHE_EXPECT_OK(WritevString(stream, std::string(100, 'a')));
  ASSERT_THAT(WritevString(stream, "x"),
              StatusIs(absl::StatusCode::kUnavailable));

  EXPECT_CALL(*visitor_raw, OnCanWrite()).Times(testing::AtLeast(1));

  InjectCapsuleOnConnectStream(
      session_id, Capsule(WebTransportMaxDataCapsule{200}));
  DeliverOnCanWriteTo(stream);
}

TEST_P(FlowControlDraft15Test,
       WritevContract_OnCanWriteNotCalledWhileDataLimitExhausted) {
  // While the session FC is still exhausted, a QUIC-level "stream is
  // writable" edge must not surface as OnCanWrite() — that would invite
  // the caller to retry into a still-blocked limit.
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  auto visitor = std::make_unique<
      testing::StrictMock<webtransport::test::MockStreamVisitor>>();
  stream->SetVisitor(std::move(visitor));
...
TEST_P(FlowControlDraft15Test,
       WritevContract_DataBlockedCapsuleSentOnFirstBlock) {
  // The adapter must emit WT_DATA_BLOCKED on the CONNECT stream so the
  // peer has a trigger to raise WT_MAX_DATA. Without this, the "try again
  // once via OnCanWrite()" promise can deadlock the writer.
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;

  QuicStreamId session_id = GetNthClientInitiatedBidirectionalId(0);
  webtransport::Stream* stream = SetUpStreamWithDataLimit(session_id, 100);
  ASSERT_NE(stream, nullptr);
  WebTransportHttp3* wt = static_cast<QuicSpdyStream*>(
                              session_->GetOrCreateStream(session_id))
                              ->web_transport();
  ASSERT_NE(wt, nullptr);
  ...
  const uint64_t bytes_before = ConnectStreamBytesWritten(session_id);
  ASSERT_THAT(WritevString(stream, "x"),
              StatusIs(absl::StatusCode::kUnavailable));
  const uint64_t bytes_after = ConnectStreamBytesWritten(session_id);

  EXPECT_GT(bytes_after, bytes_before)
      << "A WT_DATA_BLOCKED capsule should have been written on the "
         "CONNECT stream when the first WT-FC-blocked write occurred.";

  // Load-bearing companion check: a redundant MaybeSendDataBlocked() must
  // now be a no-op. 
  wt->MaybeSendDataBlocked();
  EXPECT_EQ(ConnectStreamBytesWritten(session_id), bytes_after)
      << "A second MaybeSendDataBlocked() at the same limit must not "
         "emit a duplicate capsule — proving the first block path set "
         "the data_blocked_sent_ dedup flag.";
}

TEST_P(FlowControlDraft15Test,
       WritevContract_DataBlockedCapsuleDedupedAcrossBlockedWrites) {
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  const uint64_t bytes_before_first_block = ConnectStreamBytesWritten(session_id);
  ASSERT_THAT(WritevString(stream, "x"),
              StatusIs(absl::StatusCode::kUnavailable));
  const uint64_t bytes_after_first_block = ConnectStreamBytesWritten(session_id);
  EXPECT_GT(bytes_after_first_block, bytes_before_first_block);

  ASSERT_THAT(WritevString(stream, "y"),
              StatusIs(absl::StatusCode::kUnavailable));
  EXPECT_EQ(ConnectStreamBytesWritten(session_id), bytes_after_first_block)
      << "Back-to-back blocked writes at the same WT_MAX_DATA limit must "
         "emit WT_DATA_BLOCKED exactly once.";
}

TEST_P(FlowControlDraft15Test,
       WritevContract_DataBlockedCapsuleReEmittedAfterMaxDataIncrease) {
  // The dedup flag is scoped to a single max_data_send_ value — raising
  // the limit and then hitting the new limit must emit a fresh capsule.
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  InjectCapsuleOnConnectStream(
      session_id, Capsule(WebTransportMaxDataCapsule{200}));
  QUICHE_EXPECT_OK(WritevString(stream, std::string(100, 'b')));

  const uint64_t bytes_before_second_block = ConnectStreamBytesWritten(session_id);
  ASSERT_THAT(WritevString(stream, "y"),
              StatusIs(absl::StatusCode::kUnavailable));
  const uint64_t bytes_after_second_block = ConnectStreamBytesWritten(session_id);

  EXPECT_GT(bytes_after_second_block, bytes_before_second_block)
      << "A second WT_DATA_BLOCKED capsule must be emitted after the peer "
         "raised the limit and the new limit was also reached.";
  EXPECT_GT(bytes_after_second_block, bytes_after_first_block);
}

TEST_P(FlowControlDraft15Test,
       WritevContract_NoDataBlockedCapsuleOnSuccessfulWrites) {
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  const uint64_t bytes_before = ConnectStreamBytesWritten(session_id);
  QUICHE_EXPECT_OK(WritevString(stream, std::string(20, 'a')));
  QUICHE_EXPECT_OK(WritevString(stream, std::string(30, 'b')));
  QUICHE_EXPECT_OK(WritevString(stream, std::string(40, 'c')));
  EXPECT_EQ(ConnectStreamBytesWritten(session_id), bytes_before)
      << "Successful sub-limit writes must not emit any WT_DATA_BLOCKED "
         "capsule on the CONNECT stream.";
}

TEST_P(FlowControlDraft15Test,
       WritevContract_BlockedWriteDoesNotAdvanceTotalDataSent) {
  // A blocked Writev must not consume any session FC credit. Verified
  // end-to-end: after a blocked 20-byte attempt on a 10-byte residual
  // window, raising the limit by 20 must leave room for exactly that
  // 20-byte write — proving the blocked attempt did not charge credit.
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  QUICHE_EXPECT_OK(WritevString(stream, std::string(90, 'a')));
  ASSERT_THAT(WritevString(stream, std::string(20, 'b')),
              StatusIs(absl::StatusCode::kUnavailable));

  InjectCapsuleOnConnectStream(
      session_id, Capsule(WebTransportMaxDataCapsule{120}));

  QUICHE_EXPECT_OK(WritevString(stream, std::string(20, 'b')));
  // 90 + 20 = 110 ≤ 120, so 10 more bytes must also succeed — proving
  // the blocked 20-byte attempt did not silently consume credit.
  QUICHE_EXPECT_OK(WritevString(stream, std::string(10, 'c')));
  EXPECT_FALSE(stream->CanWrite());
}

TEST_P(FlowControlDraft15Test,
       WritevContract_BlockedWriteDoesNotConsumeSlices) {
  // The all-or-nothing clause means a blocked Writev must leave the
  // caller's slices intact so the caller can retry with the same span.
  if (!VersionIsIetfQuic(GetParam().transport_version)) return;
  ...
  const std::string payload(42, 'b');
  SliceHolder holder = MakeSlice(payload);
  ASSERT_EQ(holder.slice.length(), payload.size());

  ASSERT_THAT(stream->Writev(holder.AsSpan(),
                             webtransport::kDefaultStreamWriteOptions),
              StatusIs(absl::StatusCode::kUnavailable));

  EXPECT_EQ(holder.slice.length(), payload.size())
      << "A blocked Writev() must not consume the caller's slice — the "
         "webtransport::Stream contract transfers ownership only on Ok.";
  EXPECT_FALSE(holder.slice.empty());

  InjectCapsuleOnConnectStream(
      session_id, Capsule(WebTransportMaxDataCapsule{200}));
  QUICHE_EXPECT_OK(stream->Writev(holder.AsSpan(),
                                  webtransport::kDefaultStreamWriteOptions));
}
