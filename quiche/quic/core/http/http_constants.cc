...
std::string H3SettingsToString(Http3AndQpackSettingsIdentifiers identifier) {
  switch (identifier) {
    ...
    RETURN_STRING_LITERAL(SETTINGS_WT_ENABLED);
    RETURN_STRING_LITERAL(SETTINGS_WT_INITIAL_MAX_STREAMS_UNI);
    RETURN_STRING_LITERAL(SETTINGS_WT_INITIAL_MAX_STREAMS_BIDI);
    RETURN_STRING_LITERAL(SETTINGS_WT_INITIAL_MAX_DATA);
